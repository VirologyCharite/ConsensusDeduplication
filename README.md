...
# Requirements
Requires python version 3.10 and bowtie2 version 2.5.1 and requirements.txt.

# Usage
Evertyhing is done using the `sort_reads.py` script.
```shell
$ ./sort_reads.py -h                                                                                                                                             [9:33:02]
usage: sort_reads.py [-h] [-t TARGETDIR] [--cluster-size CLUSTERSIZE] [--min-n-reads MIN_N_READS] [--max-magnitude-difference MAXMAGNITUDE] [-F] [-U]
                     FILE.sam REFERENCE.fa
...
options:
  ...
  -F, --filtered-output
                        Use this option to _not_ generate output with reads filtered.
  -U, --unfiltered-output
                        Use this option to _not_ generate output for all reads.
```
